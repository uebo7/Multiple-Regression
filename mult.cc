...
runParallel (std::vector<T> x1, std::vector<T> x2, std::vector<T> y, Input in)
{
  ...
  T finalSlope;
  finalSlope = calcIntercept (avg3, b1, avg1, b2, avg2);
  // point estimate
  T point =
...
runSerial (std::vector<T> x1, std::vector<T> x2, std::vector<T> y, Input in)
{
  ...
  timer.stop ();
  T serialTime = timer.getElapsedMs ();
  printResults ();
}
...
template<typename T>
void
printResults (const ConfidenceInterval<T> ce1,
              const ConfidenceInterval<T> ce2,
              const ConfidenceInterval<T> ce3,
              const double time)
{
